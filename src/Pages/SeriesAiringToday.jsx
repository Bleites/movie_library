...
const SeriesAiringToday = () => {
	...
	const getSeriesAiringToday = async (url) => {
		const res = await fetch(url);
		const data = await res.json();
		console.log(data);
		setAiringSeries(data.results);
		setTotalPages(data.total_pages);
	};

	useEffect(() => {
		let airingTodayUrl = `${seriesURL}airing_today?${apiKey}&page=${seriesPage}`;

		getSeriesAiringToday(airingTodayUrl);
		// console.log(moviesPage);
	}, [seriesPage]);

	return (
		<div className="container">
			<h2 className="title">On Air Series:</h2>
			<div className="movies-container">
				{console.log(airingSeries)}
				{airingSeries.length === 0 && <p>Carregando...</p>}
				{airingSeries.length > 0 &&
					airingSeries.map((serie) => (
